...
const Messages: FC<IProps> = memo(() => {
  ...
  const setValue = (event: ChangeEvent<HTMLInputElement>) => setState({name: event.target.value});
  ...
  const messages = useSelector(({messagesPage}: IAppState) => messagesPage.messages);
